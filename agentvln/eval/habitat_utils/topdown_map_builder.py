...
def convert_square_meters_to_pixel_area(
    area_m2: float, map_resolution: int, sim
) -> float:
    from .generation_utils import square_meters_to_pixel_area
...
class TopDownMapBuilder:
    def __init__(self, sim, config: dict, map_height: float = None):
        self.sim = sim
    ...
    def get_agent_map_position(self, agent_position: np.ndarray) -> np.ndarray:
    ...
    def get_visible_obstacles_map(self, agent_state, fov: float = 110.0) -> np.ndarray:
        current_point = self.get_agent_map_position(agent_state.position)
        current_angle = self.get_polar_angle(agent_state.rotation)

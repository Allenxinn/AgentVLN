...
class CoordinateTransformer:
    def __init__(self, camera_config: dict, sim):
        self.sim = sim
    ...
    def _point_to_key(self, world_point: np.ndarray) -> str:
    ...
    def map_to_world(
        self, 
        map_pixel: np.ndarray, 
        agent_position: np.ndarray,
        top_down_map: np.ndarray,
        point_height: float = None
    ) -> np.ndarray:
    ...
    def world_to_pixel(
        ...
        if point_height is None:
            project_point[1] = agent_state.position[1] - self.camera_height 
        else:
    ...
    @staticmethod
    def status_to_string(status: VisibilityStatus) -> str:
        return {

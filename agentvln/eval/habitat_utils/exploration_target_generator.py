...
class ExplorationTargetGenerator:
    ...
    def set_pixel_scale(self, meters_per_pixel: float):
    ...
    def _interpolate_contour(self, contour: np.ndarray) -> np.ndarray:
        if len(contour) < 2:
            return contour
    ...
    def _get_frontier_midpoint(self, frontier: np.ndarray) -> np.ndarray:
        if len(frontier) < 2:
    ...
    def select_metadata_with_spacing(self, candidates: List[dict]) -> List[dict]:
        selected = []

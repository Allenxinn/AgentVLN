...
from .topdown_map_builder import TopDownMapBuilder
...
class FloorMapManager:
    def __init__(self, sim, map_config: dict, initial_height: float = None):
        self.sim = sim
    ...
    def _matching_floor(self, height: float) -> Optional[int]:
        matches = [
    ...
    def _ensure_transition_builder(self, height: float, agent_position) -> None:
        if (
    ...
    def _build_valid_floor(
        self, height: float, agent_position
    ...
    def update_visibility(self, agent_state, fov: float = 110.0):

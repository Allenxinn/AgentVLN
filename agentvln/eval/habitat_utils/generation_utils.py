...
def get_actuation_noise_settings(config: Dict[str, Any]) -> Dict[str, Any]:
...
def build_generation_config(config: Dict[str, Any]) -> Dict[str, Any]:
...
def dataset_actuation_noise_settings(generation_config: Dict[str, Any]):
...
def square_meters_to_pixel_area(
    area_m2: float, meters_per_pixel: float
) -> float:
...
def densify_polyline(
    points: Iterable[np.ndarray], interval: float = 0.25
) -> List[np.ndarray]:
...
def build_geodesic_path(
    sim,
    start,
    ordered_targets: Iterable[np.ndarray],
    interval: float = 0.25,
...
def validate_dataset_contract(dataset: Dict[str, Any], expected_config=None) -> None:
